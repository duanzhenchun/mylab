...
def get_follow(cur, Id, date_limit=None):
    ...
    cmd = "select followers_count from Weibo_User_Profile_%d where id=%d and (followers_count<200000 or friends_count<2000) %s" % (
        Id % 10, Id, lim)
    cur.execute(cmd)
    nfol = -1
    res = cur.fetchone()
...
def candidates_table(tblname):
    ...
    while True:
        ...
        try:
            ...
            f_err.write('%s,%s,%s\n' % (uid, orguid, e))
            continue
        if nfol < org_nfol or (nfol > 0 and
                               random.uniform(0, 1) < org_nfol / float(nfol)):
...
def filter_lasttweet(fname='./out_new2.txt'):
    ...
    for l in f:
        uid = l.strip()
        cmd = 'select 1 from Weibo_User_Profile where id=%s and last_tweet_publish_time>"%s" limit 1' % uid
        cur.execute(cmd)
        res = cur.fetchone()
...
def copy_dbrow(fname):
    ...
    for l in f:
        uid = int(l.strip())
        cmd = 'insert ignore into Weibo_filter_User_Profile (select * from Weibo_User_Profile_%d where id=%d)' % (
            uid % 10, uid)
        cur.execute(cmd)
