...
def slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    #make the constraint info matrix and vec
    Omega=matrix()
    Omega.zero(dim, dim)
    Omega.value[0][0]=1.0
    Omega.value[1][1]=1.0
    ...
    for k in range(len(data)):
        # n is index of robot pos 
        n=k*2
        measurement=data[k][0]
        motion  = data[k][1]
        for i in range(len(measurement)):
            # m is index of landmark coordinate in the matrix/vec
            m=2*(N+measurement[i][0])
            #update the info matrix/vec based on measurement
            for b in range(2):
    ...
    #update info  matrix/vec based on robot motion
    for b in range(4):
