...
class Encoder:  
    def __init__(self, mode=SPEEX_MODEID_NB):  
        self.state = spx.speex_encoder_init(spx.speex_nb_mode)
...
#        self.set(SPEEX_SET_COMPLEXITY, 3)  
        ...
        self.bits = SpeexBits()  
        spx.speex_bits_init(byref(self.bits))  
    
    def __del__(self):  
        spx.speex_encoder_destroy(self.state)  
        spx.speex_bits_destroy(byref(self.bits))  
          
    def encode(self, string_audio_data):  
        short_d = np.fromstring(string_audio_data, dtype=np.short) 
        spx.speex_bits_reset(byref(self.bits))  
        spx.speex_encode_int(self.state, short_d.ctypes.data_as(POINTER(c_short)), byref(self.bits))  
        towrite = spx.speex_bits_nbytes(byref(self.bits))
        buf=create_string_buffer(towrite)
        spx.speex_bits_write(byref(self.bits), buf, towrite) 
        res = chr(towrite) + buf.raw
        return res  
    ...
    def set(self, request, val):  
        if request in [SPEEX_SET_VBR, SPEEX_SET_QUALITY, SPEEX_SET_COMPLEXITY, SPEEX_SET_HIGHPASS]:  
            tmp = c_int()  
            tmp.value = val  
            spx.speex_encoder_ctl(self.state, request, byref(tmp))  
  
    def get(self, request):  
        if request in [SPEEX_GET_FRAME_SIZE,SPEEX_GET_MODE]:  
            tmp = c_int()  
            spx.speex_encoder_ctl(self.state, request, byref(tmp))  
            return tmp.value  
        else:  
...
class Decoder:  
    ...
    def __del__(self):  
        spx.speex_decoder_destroy(self.state)  
        spx.speex_bits_destroy(byref(self.bits))  
    ...
    def decode(self, data):  
        ...
        buf.value = data
        spx.speex_bits_read_from(byref(self.bits), buf, size) 
        #buftype = c_short * size
        #sht_buf = buftype()
        #short_d=np.ndarray 
        spx.speex_decode_int(self.state, byref(self.bits), buf)
        print buf
        #return short_d.tostring()
